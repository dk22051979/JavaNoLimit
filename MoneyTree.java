  							 ...
  							 implements ActionListener, TreeSelectionListener
{
  ...
  private static String[] RbiTables = new String[5],
						...
						dates = new String[14000],
						ohlc = new String[14000],
						...
						nse_etf1 = {
							"GOLDBEES_eq", "KOTAKGOLD_eq", "GOLDSHARE_eq",
							"SBIGETS_eq", "HDFCMFGETF_eq", "AXISGOLD_eq",
							"RELGOLD_eq", "IDBIGOLD_eq",
							"RELIGAREGO_eq", "BSLGOLDETF_eq",
							"IPGETF_eq", "MGOLD_eq","CRMFGETF_eq"
						},
                        nse_etf2 = {
							"QGOLDHALF_eq"
						},
                        nse_etf3 = {
							"NIFTYBEES_eq",	"KOTAKNIFTY_eq", "RELGRNIFTY_eq",
							"QNIFTY_eq", "IIFLNIFTY_eq"
						},
						nse_etf4 = {
                            "N100_eq"
						},
                        ...
                        nse_etf6 = {
							"BANKBEES_eq", "RELBANK_eq"
						},
						nse_etf7 = {
							"KOTAKPSUBK_eq", "PSUBNKBEES_eq"
						},
						nse_etf8 = {
							"SHARIABEES_eq"
						},
  ...
  private static int totalRecords,
					...
					bsecols, nsecols,
					rowsOrder;
  private static double[] notSorted = new double[51],
  						  Sorted = new double[51];
  private static int[] idxSorted = new int[51];
  private static String[] sortedScrips  = new String[51];
  ...
  private JButton fetchButton, fetchButton2, showConnectionInfoButton, showConnectionInfoButton2,
	  		btnMyEquityInsert, btnNasdaq, btnHangSeng, btnDax,
			btnSsecomposite, btnNikkei, btnCac, btnFtse, btnST;
  private JLabel lblRbi, lblNifty, lblBseEquity, lblNseEquity, lblNseFno, lblMcxFut, lblNcdexFut, lblNseOpt,
			...
			lblNasdaq, lblHangSeng, lblDax, lblDateWorldIndices,
			lblSsecomposite, lblNikkei, lblCac, lblFtse, lblST,
			lblNPCDate, lblNPCDateFrom,
			lblNNPCDate, lblNNPCDateFrom,
			lblNPCSDate, lblNPCSDateFrom,
			lblNNPCSDate, lblNNPCSDateFrom,
			lblNODate, lblNODateFrom,
			lblNNODate, lblNNODateFrom;
  private JTextField txtRbiFileName, txtNiftyFileName, txtBseEquityFileName, txtNseEquityFileName, txtNseFnoFileName,
			...
			textNasdaq, textHangSeng, textDax, textDateWorldIndices,
			textSsecomposite, textNikkei, textCac, textFtse, textST,
			textNPCDate, textNPCDateFrom,
			textNNPCDate, textNNPCDateFrom,
			textNPCSDate, textNPCSDateFrom,
			textNNPCSDate, textNNPCSDateFrom,
			textNODate, textNODateFrom,
			textNNODate, textNNODateFrom;
  ...
  public MoneyTree(String appTitle)
  {
    ...
    //getFromCsvFile(NSE_DB);
    //clear_database(NSE_DB, "nse_", NseEqTables, "'2012-04-24'");
    //create_table(FUNDAMENTAL_DB, "nonniftydetails", nonNifty);
  }
  ...
  public JPanel createContent()
  {
    ...
    tabbedPaneApp.add("NiftyScripDetails", ShowTabNiftyScripDetailsPanel());
   ...
   // tabbedPaneApp.add("Query CMP%", ShowTabQueryPercentagePanel());
    ...
    tabbedPaneApp.add("GOLD", ShowTabNseEtfPanel1());
    tabbedPaneApp.add("GOLD HALF", ShowTabNseEtfPanel2());
    tabbedPaneApp.add("NIFTY", ShowTabNseEtfPanel3());
    tabbedPaneApp.add("N100", ShowTabNseEtfPanel4());
    tabbedPaneApp.add("HANGSENG", ShowTabNseEtfPanel5());
    tabbedPaneApp.add("BANK", ShowTabNseEtfPanel6());
    tabbedPaneApp.add("PSUBANK", ShowTabNseEtfPanel7());
    tabbedPaneApp.add("SHARIA", ShowTabNseEtfPanel8());
    tabbedPaneApp.add("M100", ShowTabNseEtfPanel9());
    tabbedPaneApp.add("M50", ShowTabNseEtfPanel10());
    tabbedPaneApp.add("LIQUID", ShowTabNseEtfPanel11());
    tabbedPaneApp.add("JUNIOR", ShowTabNseEtfPanel12());
    tabbedPaneApp.add("INFRA", ShowTabNseEtfPanel13());
    tabbedPaneApp.add("NO", ShowTabNOPanel());
    tabbedPaneApp.add("NPC", ShowTabNPCPanel());
    tabbedPaneApp.add("NNPC", ShowTabNNPCPanel());
    tabbedPaneApp.add("NPCS", ShowTabNPCSPanel());
    tabbedPaneApp.add("NNPCS", ShowTabNNPCSPanel());
...
//    localDefaultMutableTreeNode.add(createNodeName2("FIIDII"));
  ...
  public JPanel ShowTabInsertDataPanel()
  {
	...
	lblNasdaq = new JLabel("   Nasdaq");
	textNasdaq = new JTextField("", 20);
	lblHangSeng = new JLabel("   HangSeng");
	textHangSeng = new JTextField("", 20);
	lblDax = new JLabel("   Dax");
	textDax = new JTextField("", 20);
	lblSsecomposite = new JLabel("   Ssecomposite");
	textSsecomposite = new JTextField("", 20);
	lblST = new JLabel("   StraitTimes");
	textST = new JTextField("", 20);
	lblNikkei = new JLabel("   Nikkei");
	textNikkei = new JTextField("", 20);
	lblCac = new JLabel("   Cac");
	textCac = new JTextField("", 20);
	lblFtse = new JLabel("   Ftse");
	textFtse = new JTextField("", 20);
	...
	JLabel dummy = new JLabel("file.csv / o,h,l,c,v");
	btnNasdaq = new JButton("USA Nasdaq");
	btnHangSeng = new JButton("HongKong HangSeng");
	btnDax = new JButton("Europe DAX");
	btnSsecomposite = new JButton("China Ssecomposite");
	btnNikkei = new JButton("Japan Nikkei");
	btnCac = new JButton("Europe Cac");
	btnFtse = new JButton("UK Ftse");
	btnST = new JButton("Singapore StraitTimes");
	...
	indicesPanel.add(lblNasdaq);
	indicesPanel.add(textNasdaq);
	indicesPanel.add(btnNasdaq);
	indicesPanel.add(lblFtse);
	indicesPanel.add(textFtse);
	indicesPanel.add(btnFtse);
	indicesPanel.add(lblDax);
	indicesPanel.add(textDax);
	indicesPanel.add(btnDax);
	indicesPanel.add(lblCac);
	indicesPanel.add(textCac);
	indicesPanel.add(btnCac);
	indicesPanel.add(lblSsecomposite);
	indicesPanel.add(textSsecomposite);
	indicesPanel.add(btnSsecomposite);
	indicesPanel.add(lblNikkei);
	indicesPanel.add(textNikkei);
	indicesPanel.add(btnNikkei);
	indicesPanel.add(lblHangSeng);
	indicesPanel.add(textHangSeng);
	indicesPanel.add(btnHangSeng);
	indicesPanel.add(lblST);
	indicesPanel.add(textST);
	indicesPanel.add(btnST);

	insertDataPanel.add(filesPanel);
	insertDataPanel.add(indicesPanel);

	   textNasdaq.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textNasdaq.getText());
	                textNasdaq.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   btnNasdaq.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("Nasdaq", textNasdaq.getText());
	            btnNasdaq.setBackground(new Color(255, 0, 0));
	        }
	   });
	   textHangSeng.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textHangSeng.getText());
	                textHangSeng.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   btnHangSeng.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("HangSeng", textHangSeng.getText());
	            btnHangSeng.setBackground(new Color(255, 0, 0));
	        }
	   });
	   textDax.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textDax.getText());
	                textDax.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   btnDax.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("Dax", textDax.getText());
	            btnDax.setBackground(new Color(255, 0, 0));
	        }
	   });
	   textSsecomposite.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textSsecomposite.getText());
	                textSsecomposite.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   btnSsecomposite.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("Ssecomposite", textSsecomposite.getText());
	        	btnSsecomposite.setBackground(new Color(255, 0, 0));
			}
	   });
	   textNikkei.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textNikkei.getText());
	                textNikkei.setBackground(new Color(255, 0, 255));
	            }
	        }
	   ...
	   textCac.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textCac.getText());
	                textCac.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   textST.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textST.getText());
	                textST.setBackground(new Color(255, 0, 255));
	            }
	        }
	   ...
	   btnST.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("StraitTimes", textST.getText());
	            btnST.setBackground(new Color(255, 0, 0));
	        }
	   });
	   btnCac.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("Cac", textCac.getText());
	            btnCac.setBackground(new Color(255, 0, 0));
	        }
	   });
	   textFtse.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	                insertWorldIndexCsvFileToDatabase(textFtse.getText());
	                textFtse.setBackground(new Color(255, 0, 255));
	            }
	        }

	    }
	   );
	   btnFtse.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	            insertWorldIndicesToDatabase("Ftse", textFtse.getText());
	            btnFtse.setBackground(new Color(255, 0, 0));
	        }
	   });
  ...
  public JPanel ShowTabNiftyScripDetailsPanel() {
       ...
       cmbNifty.addActionListener(new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                     ...
                     texaNews.setText(temp[8]);
                     String[] qtrs = null;
					 qtrs = temp[9].split(">");
                     texaProfitLoss.setText(qtrs[0] + "\t" + qtrs[1] + "\t" + qtrs[2] + "\t" + qtrs[3] + "\n" + qtrs[4] + "\t" + qtrs[5] + "\t" + qtrs[6] + "\t" + qtrs[7] + "\n");
              }
       });
  ...
  public JPanel ShowTabNseEtfPanel1()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf1), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel2()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf2), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel3()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf3), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel4()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf4), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel5()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf5), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel6()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf6), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel7()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf7), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel8()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf8), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel9()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf9), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel10()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf10), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel11()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf11), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel12()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf12), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNseEtfPanel13()
  {
  	JPanel main = new JPanel(new BorderLayout());
  	main.add(createNiftyChart(nse_etf13), BorderLayout.CENTER);
  	return main;
  }
  public JPanel ShowTabNOPanel()
  {
	JPanel pnlRoot = new JPanel(new BorderLayout()),
			pnlInsert = new JPanel(new GridLayout(1, 4));
	pnlRoot.setPreferredSize(new Dimension(800,400));
	pnlRoot.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));

	lblNODate = new JLabel("   Enter Pchange date To DB");
	textNODate = new JTextField("'" + C_YEAR + "-" + monthn[calendar.get(Calendar.MONTH)] + "-" + dayn[calendar.get(Calendar.DAY_OF_MONTH)] + "'", 10 );
	lblNODateFrom = new JLabel("   Enter Pchange date To Retrieve");
	textNODateFrom = new JTextField(PC_FROM);
	pnlInsert.add(lblNODate);
	pnlInsert.add(textNODate);
	pnlInsert.add(lblNODateFrom);
	pnlInsert.add(textNODateFrom);

	JTable tblLocal = new JTable(new NOTableModel());
	TableCellRenderer NOrenderer = new NOTableCellRenderer();
    tblLocal.setDefaultRenderer( String.class, NOrenderer );

	tblLocal.setAutoCreateRowSorter(true);
	tblLocal.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
	JScrollPane scpaneLocal = new JScrollPane(tblLocal);
	pnlRoot.add(pnlInsert, BorderLayout.NORTH);
	pnlRoot.add(scpaneLocal, BorderLayout.CENTER);

	textNODate.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
		        	insertNOToDatabase(textNODate.getText());
		        	textNODate.setBackground(Color.MAGENTA);
		        	System.out.println(textNODate.getText());
	            }
	        }

	    }
	);
	textNODateFrom.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
                 	System.out.println(textNODateFrom.getText());
	            }
	        }
  ...
  public JPanel ShowTabNPCPanel()
  {
	...
	lblNPCDate = new JLabel("   Enter Pchange date To DB");
	textNPCDate = new JTextField("'" + C_YEAR + "-" + monthn[calendar.get(Calendar.MONTH)] + "-" + dayn[calendar.get(Calendar.DAY_OF_MONTH)] + "'", 10 );
	lblNPCDateFrom = new JLabel("   Enter Pchange date To Retrieve");
	textNPCDateFrom = new JTextField(PC_FROM);
	pnlInsert.add(lblNPCDate);
	pnlInsert.add(textNPCDate);
	pnlInsert.add(lblNPCDateFrom);
	pnlInsert.add(textNPCDateFrom);

	JTable tableNiftyPC = new JTable(new NPCTableModel());
	TableCellRenderer NiftyPCrenderer = new PCTableCellRenderer();
	...
	pnlRoot.add(pnlInsert, BorderLayout.NORTH);
	pnlRoot.add(scrollpaneTableNiftyPC, BorderLayout.CENTER);

	textNPCDate.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
					getNiftyIndex(textNPCDate.getText());
	            	calculateIndexPchange();
		    		for(int t = 0; t < nifty50.length; t++)
						getNiftyCMPs("nse_" + nifty50[t], textNPCDate.getText());
					calculatePchange(nifty50.length);
		        	insertPCToDatabase(textNPCDate.getText());
		        	textNPCDate.setBackground(Color.BLUE);
		        	System.out.println(textNPCDate.getText());
	            }
	        }

	    }
	);
	textNPCDateFrom.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
                 	System.out.println(textNPCDateFrom.getText());
	            }
	        }
  ...
  public JPanel ShowTabNNPCPanel()
  {
	JPanel pnlRoot = new JPanel(new BorderLayout()),
			pnlInsert = new JPanel(new GridLayout(1, 4));
	pnlRoot.setPreferredSize(new Dimension(800,400));
	...
	lblNNPCDate = new JLabel("   Enter Pchange date To DB");
	textNNPCDate = new JTextField("'" + C_YEAR + "-" + monthn[calendar.get(Calendar.MONTH)] + "-" + dayn[calendar.get(Calendar.DAY_OF_MONTH)] + "'", 10 );
	lblNNPCDateFrom = new JLabel("   Enter Pchange date To Retrieve");
	textNNPCDateFrom = new JTextField(PC_FROM);
	pnlInsert.add(lblNNPCDate);
	pnlInsert.add(textNNPCDate);
	pnlInsert.add(lblNNPCDateFrom);
	pnlInsert.add(textNNPCDateFrom);

	JTable tableNiftyPC = new JTable(new NNPCTableModel());
	TableCellRenderer NiftyPCrenderer = new PCTableCellRenderer();
    tableNiftyPC.setDefaultRenderer( Double.class, NiftyPCrenderer );
	...
	textNNPCDate.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	            	getNiftyIndex(textNNPCDate.getText());
	            	calculateIndexPchange();
		    		for(int t = 0; t < nonNifty.length; t++) {
						getNiftyCMPs("nse_" + nonNifty[t], textNNPCDate.getText());
					}
					calculatePchange(nonNifty.length);
		        	insertNNPCToDatabase(textNNPCDate.getText());
		        	textNNPCDate.setBackground(Color.GREEN);
		        	System.out.println(textNNPCDate.getText());
	            }
	        }

	    }
	);
	textNNPCDateFrom.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
                 	System.out.println(textNNPCDateFrom.getText());
	            }
	        }
  ...
  public JPanel ShowTabNPCSPanel()
  {
	JPanel pnlRoot = new JPanel(new BorderLayout()),
			pnlInsert = new JPanel(new GridLayout(1, 4));
	pnlRoot.setPreferredSize(new Dimension(800,400));
	...
	lblNPCSDate = new JLabel("   Enter Pchange date To DB");
	textNPCSDate = new JTextField("'" + C_YEAR + "-" + monthn[calendar.get(Calendar.MONTH)] + "-" + dayn[calendar.get(Calendar.DAY_OF_MONTH)] + "'", 10 );
	lblNPCSDateFrom = new JLabel("   Enter Pchange date To Retrieve");
	textNPCSDateFrom = new JTextField(PC_FROM);
	pnlInsert.add(lblNPCSDate);
	pnlInsert.add(textNPCSDate);
	pnlInsert.add(lblNPCSDateFrom);
	pnlInsert.add(textNPCSDateFrom);

	JTable tblLocal = new JTable(new NPCSTableModel());
	TableCellRenderer NiftyPCrenderer = new PCTableCellRenderer();
    tblLocal.setDefaultRenderer( Double.class, NiftyPCrenderer );
	...
	textNPCSDate.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	            /*
	            	getNiftyIndex(textNPCSDate.getText());
	            	calculateIndexPchange();
		    		for(int t = 0; t < nifty50.length; t++)
						getNiftyCMPs("nse_" + nifty50[t], textNPCSDate.getText());
					calculatePchange(nifty50.length);
		        	insertPCToDatabase(textNPCSDate.getText());
		        */
		        	textNPCSDate.setBackground(Color.RED);
		        	System.out.println(textNPCSDate.getText());
	            }
	        }

	    }
	);
	textNPCSDateFrom.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
                 	System.out.println(textNPCSDateFrom.getText());
	            }
	        }
  ...
  public JPanel ShowTabNNPCSPanel()
  {
	JPanel pnlRoot = new JPanel(new BorderLayout()),
			pnlInsert = new JPanel(new GridLayout(1, 4));
	pnlRoot.setPreferredSize(new Dimension(800,400));
	...
	lblNNPCSDate = new JLabel("   Enter Pchange date To DB");
	textNNPCSDate = new JTextField("'" + C_YEAR + "-" + monthn[calendar.get(Calendar.MONTH)] + "-" + dayn[calendar.get(Calendar.DAY_OF_MONTH)] + "'", 10 );
	lblNNPCSDateFrom = new JLabel("   Enter Pchange date To Retrieve");
	textNNPCSDateFrom = new JTextField(PC_FROM);
	pnlInsert.add(lblNNPCSDate);
	pnlInsert.add(textNNPCSDate);
	pnlInsert.add(lblNNPCSDateFrom);
	pnlInsert.add(textNNPCSDateFrom);

	JTable tblLocal = new JTable(new NNPCSTableModel());
	TableCellRenderer NiftyPCrenderer = new PCTableCellRenderer();
    tblLocal.setDefaultRenderer( Double.class, NiftyPCrenderer );
	...
	textNNPCSDate.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
	            /*
	            	getNiftyIndex(textNNPCSDate.getText());
	            	calculateIndexPchange();
		    		for(int t = 0; t < nonNifty.length; t++) {
						getNiftyCMPs("nse_" + nonNifty[t], textNNPCSDate.getText());
					}
					calculatePchange(nonNifty.length);
		        	insertNonNiftyPchangeToDatabase(textNNPCSDate.getText());
		        */
		        	textNNPCSDate.setBackground(Color.CYAN);
		        	System.out.println(textNNPCSDate.getText());
	            }
	        }

	    }
	);
	textNNPCSDateFrom.addKeyListener(new KeyAdapter() {
	        public void keyPressed(KeyEvent ke) {
	            int key =ke.getKeyCode();
	            if(key==KeyEvent.VK_ENTER){
                 	System.out.println(textNNPCSDateFrom.getText());
	            }
	        }
  ...
  public void getNiftyCMPs(String tableName, String trdate)
  {
	...
	public void insertNOToDatabase(String trdate) {
        getOrder(trdate);
		System.arraycopy(notSorted, 0, Sorted, 0, 51);
    	Arrays.sort(Sorted);
        for(int u = 0; u < 51; ++u) {
			idxSorted[u] = Arrays.binarySearch(Sorted, notSorted[u]);
			sortedScrips[idxSorted[u]] = nifty50[u].substring(0,nifty50[u].indexOf("_"));
		}
		String allscr = "", sql ="";
		for(int u = 50; u >= 0; --u) allscr += "," + sortedScrips[u];
		sql = "INSERT INTO niftyorder VALUES ( null, " + trdate + ",'" + allscr.substring(1) + "'" + " )";
		//System.out.println(beg);
		try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + FUNDAMENTAL_DB, "root","deepak");
            Statement statement = connection.createStatement();
	...
	public void insertPCToDatabase(String trdate)
	{
	...
	public void insertNNPCToDatabase(String trdate)
	{
  ...
  public void getOrder(String tdate)
  {
      try {
        Class.forName("com.mysql.jdbc.Driver");
        totalRecords = 0;
        Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + FIIDII_DB, "root","deepak");
        Statement statement = connection.createStatement();
        String sql = "SELECT * FROM niftypc WHERE tradedate = " + tdate;
		ResultSet rs = statement.executeQuery(sql);
        while (rs.next()) {
			for(int y = 0; y < nifty50.length; ++y) {
				notSorted[y] = rs.getDouble(nifty50[y].substring(0,nifty50[y].indexOf("_")));
			}
		}
        connection.clearWarnings();
        connection.close();
  ...
  public TimeSeries createNiftyLowsTimeSeries(String scrip)
 {
  ...
  class NOTableModel extends AbstractTableModel implements TableModelListener {
        private String[] cols;
        ...
        public NOTableModel() {
        	...
        	cols[0] = "ID";
        	cols[1] = "TRADEDATE";
        	for(int i =1; i < 52; ++i) cols[i+1] = String.valueOf(i);
			Vector rawdata = new Vector();
			int vector_index = 0;
            rawdata = generateNOTableDataInVector(textNODateFrom.getText());
            tbdata = new Object[rowsOrder][cols.length];
            for(int r = 0; r < rowsOrder; r++) {
            	for(int c = 0; c < cols.length; c++) {
					tbdata[r][c] = rawdata.get(vector_index++);
            	}
            }
 		}
  ...
  public Vector generateNOTableDataInVector(String tdate) {
		Vector r = new Vector();
		rowsOrder = 0;
		try {
		    Class.forName("com.mysql.jdbc.Driver");
		    Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + FUNDAMENTAL_DB, "root","deepak");
		    Statement statement = connection.createStatement();
		    String sql = "SELECT * FROM niftyorder WHERE tradedate > " + tdate + " ORDER BY tradedate";
		    ResultSet rs = statement.executeQuery(sql);
		    while (rs.next()) {
                ...
                r.add(tdate1);
                String scriporder1 = rs.getString("scriporder");
                String[] scrips = scriporder1.split(",");
                for(int i = 0; i < 51; ++i) r.add(scrips[i]);
                rowsOrder++;
		    }
		    connection.close();
		} catch(Exception e) {
  ...
  class NPCTableModel extends AbstractTableModel implements TableModelListener {
		...
		public  NPCTableModel(){
			...
			int vector_index = 0;
            rawdata = generateNPCTableDataInVector(textNPCDateFrom.getText());
  ...
  class NNPCTableModel extends AbstractTableModel implements TableModelListener {
        private String[] columnNames2;
        private Vector rawdata;
        private Object[][] data;
		public  NNPCTableModel(){
			...
			this.columnNames2 = col2;
			int vector_index = 0;
            rawdata = generateNNPCTableModelDataInVector(textNNPCDateFrom.getText());
  ...
  public Vector generateNPCTableDataInVector(String tradedate) {
		Vector r = new Vector();
  ...
  public Vector generateNNPCTableModelDataInVector(String tradedate) {
		Vector r = new Vector();
  ...
  class NPCSTableModel extends AbstractTableModel implements TableModelListener {
        private String[] columnNames2;
        private Vector rawdata;
        private Object[][] data;
		public  NPCSTableModel(){
			String[] col2 = new String[55];
			col2[0] = "id";
			col2[1] = "tradedate";
			col2[2] = "NIFTY";
			...
			this.columnNames2 = col2;
			int vector_index = 0;
            rawdata = generateNPCSTableDataInVector(textNPCSDateFrom.getText());
            data = new Object[noOfRowsNiftyPCTable][columnNames2.length];
            for(int r = 0; r < noOfRowsNiftyPCTable; r++) {
            	for(int c = 0; c < columnNames2.length; c++) {
					data[r][c] = rawdata.get(vector_index++);
  ...
  class NNPCSTableModel extends AbstractTableModel implements TableModelListener {
        private String[] columnNames2;
        private Vector rawdata;
        private Object[][] data;
		public  NNPCSTableModel(){
			String[] col2 = new String[nonNifty.length + 4];
			col2[0] = "id";
			col2[1] = "tradedate";
			col2[2] = "NIFTY";
			...
			this.columnNames2 = col2;
			int vector_index = 0;
            rawdata = generateNNPCSTableDataInVector(textNNPCSDateFrom.getText());
            data = new Object[noOfRowsNonNiftyPCTable][columnNames2.length];
            for(int r = 0; r < noOfRowsNonNiftyPCTable; r++) {
            	for(int c = 0; c < columnNames2.length; c++) {
					data[r][c] = rawdata.get(vector_index++);
  ...
  public Vector generateNPCSTableDataInVector(String tradedate) {
		Vector r = new Vector();
		noOfRowsNiftyPCTable = 0;
		try {
		    Class.forName("com.mysql.jdbc.Driver");
  ...
  public Vector generateNNPCSTableDataInVector(String tradedate) {
		Vector r = new Vector();
		noOfRowsNonNiftyPCTable = 0;
		try {
		    Class.forName("com.mysql.jdbc.Driver");
  ...
  class PCTableCellRenderer extends DefaultTableCellRenderer
	{
	    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
	    {
			...
			if(column > 1) {
				...
				if(cellInt <= 0) {
					...
					else if(cellInt <= -1 && cellInt > -3) cell.setBackground(new Color(255, 225, 255));
	   		        else if(cellInt <= -3 && cellInt > -5) cell.setBackground(new Color(255, 215, 255));
	   		        else if(cellInt <= -5 && cellInt > -7) cell.setBackground(new Color(255, 205, 255));
	   		        else if(cellInt <= -7 && cellInt > -9) cell.setBackground(new Color(255, 195, 255));
	   		        else if(cellInt <= -9 && cellInt > -11) cell.setBackground(new Color(255, 185, 255));
	   		        else if(cellInt <= -11 && cellInt > -13) cell.setBackground(new Color(255, 175, 255));
	   		        else if(cellInt <= -13 && cellInt > -15) cell.setBackground(new Color(255, 165, 255));
	   		        else if(cellInt <= -15 && cellInt > -17) cell.setBackground(new Color(255, 155, 255));
	   		        else if(cellInt <= -17 && cellInt > -19) cell.setBackground(new Color(255, 145, 255));
	   		        else if(cellInt <= -19 && cellInt > -21) cell.setBackground(new Color(255, 135, 255));
	   		        else if(cellInt <= -21 && cellInt > -31) cell.setBackground(new Color(255, 125, 255));
	   		        else if(cellInt <= -31 && cellInt > -101) cell.setBackground(new Color(255, 115, 255));
	   		        else cell.setBackground(new Color(255, 105, 255));
	   		    } else {
					if(cellInt >= 1 && cellInt < 3) cell.setBackground(new Color(225, 255, 255));
			        else if(cellInt >= 3 && cellInt < 5) cell.setBackground(new Color(215, 255, 255));
	   		        else if(cellInt >= 5 && cellInt < 7) cell.setBackground(new Color(205, 255, 255));
	   		        else if(cellInt >= 7 && cellInt < 9) cell.setBackground(new Color(195, 255, 255));
	   		        else if(cellInt >= 9 && cellInt < 11) cell.setBackground(new Color(185, 255, 255));
	   		        else if(cellInt >= 11 && cellInt < 13) cell.setBackground(new Color(175, 255, 255));
	   		        else if(cellInt >= 13 && cellInt < 15) cell.setBackground(new Color(165, 255, 255));
	   		        else if(cellInt >= 15 && cellInt < 17) cell.setBackground(new Color(155, 255, 255));
	   		        else if(cellInt >= 17 && cellInt < 19) cell.setBackground(new Color(145, 255, 255));
	   		        else if(cellInt >= 19 && cellInt < 21) cell.setBackground(new Color(135, 255, 255));
	   		        else if(cellInt >= 21 && cellInt < 31) cell.setBackground(new Color(125, 255, 255));
	   		        else if(cellInt >= 31 && cellInt < 101) cell.setBackground(new Color(115, 255, 255));
	   		        else cell.setBackground(new Color(105, 255, 255));
	   		    }
		    } else {
  ...
  class NOTableCellRenderer extends DefaultTableCellRenderer
	{
	    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
	    {
	        Component cell = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
			if(column < 13) {
		    	cell.setBackground(new Color(105, 255, 255));
		    } else if(column >= 13 && column < 23) {
				cell.setBackground(new Color(185, 255, 255));
		    } else if(column >= 23 && column < 33) {
				cell.setBackground(new Color(255, 215, 255));
		    } else if(column >= 33 && column < 43) {
				cell.setBackground(new Color(255, 135, 255));
		    } else {
				cell.setBackground(new Color(255, 75, 255));
		    }
	        return cell;
	    }
	}
  ...
  public void insertWorldIndexCsvFileToDatabase(String fileName)
  {
     try
     {
            BufferedReader br = new BufferedReader( new FileReader(fileName));
            ...
            while( (strLine = br.readLine()) != null)
            {
                ...
                if(count > 0)
                {
                        ...
                        date1 = new String(columns[0].trim());
                        //System.out.println(columns[0].trim());
                        String[] dt = date1.split("-");
                        String m =new String(dt[1].trim());
                        String d =new String(dt[2].trim());
                        String y =new String(dt[0].trim());
                        String myDate =  y + "-" + m + "-" + d;
                        ...
                        oi = new String(columns[5].replaceAll("\"","").trim());
                        String ohlcv = o + "," + h + "," + l + "," + c + "," + oi;
             			String sql = "INSERT INTO " + tableName + " (id, tradedate, ohlc) VALUES (null,'"  + myDate +  "','" + ohlcv + "')";
            			//System.out.println(sql);
						statement.executeUpdate(sql);
                }
                ++count;
  ...
  public void insertWorldIndicesToDatabase(String tableName, String v)
  {
     	try
     	{
            Class.forName("com.mysql.jdbc.Driver");
            Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + WORLDINDICES_DB, "root","deepak");
            Statement statement = connection.createStatement();
            String myDate =new String(textDateWorldIndices.getText()), ohlcv = new String(v);
    ...
    throws IOException
  {
   ...
   public void connect2() {
	...
	public void clear_database(String db, String prefix, String[] scrips, String trdate) {
		for(int idx =0 ;scrips[idx++] != null;) {
			try {
				Class.forName("com.mysql.jdbc.Driver");
				Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + db, "root","deepak");
				Statement statement = connection.createStatement();
				String sql = "DELETE FROM " + prefix + scrips[idx] + " WHERE tradedate = " + trdate;
				System.out.println(sql);
				statement.executeUpdate(sql);
				connection.close();
			}catch(Exception e) {
	...
	public void create_table(String db, String tbname, String[] scrips) {
		//String beg = "CREATE TABLE " + tbname + "( id int auto_increment, bdate date, quarter varchar(50), ", sql = "", tmp = "", end = " primary key(id))";
		//for(int idx = 0 ; idx < scrips.length; ++idx) { tmp += scrips[idx].substring(0, scrips[idx].indexOf("_")) + " varchar(100), "; }
		String beg = "CREATE TABLE " + tbname + "( id int auto_increment, ", sql = "", tmp = "", end = " primary key(id))";
		tmp = "scrip varchar(100), company varchar(100),lot int,sector varchar(100),fv int,agm date,uri varchar(100),news varchar(1000), ";
			try {
				Class.forName("com.mysql.jdbc.Driver");
				Connection connection = DriverManager.getConnection("jdbc:mysql://localhost/" + db, "root","deepak");
				Statement statement = connection.createStatement();
